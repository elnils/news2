...
v10 (additiv, alle alten Felder unveraendert):
    herkunft   medien | parlament | eu-inst | laender | gericht | think
    gericht    EuGH, BVerfG, BGH, BVerwG, BAG, BSG, BFH, EuG, Supreme Court
    az         Aktenzeichen der Entscheidung (C-412/24, 2 BvR 1188/26, ...)
    ls         Leitsatz bzw. erster Satz der Pressemitteilung
    personen   IDs aus people.json, die in der Meldung vorkommen
  plus Feeds fuer BVerwG, BAG, BSG, BFH und das Gericht der EU.
"""
...
EU_OFFICIAL_FEEDS = [
    ...
    # Weitere Denkfabriken und Wirtschaftsinstitute. Alle mit eigenem Feed;
    # wo keiner existiert, ueber die Nachrichtensuche (wie bei SWP und Bruegel).
    ("https://dgap.org/de/rss.xml",                                              "DGAP",                 "eu-think"),
    ("https://www.delorscentre.eu/en/feed",                                      "Jacques Delors Centre","eu-think"),
    ("https://www.ceps.eu/feed/",                                                "CEPS Brüssel",         "eu-think"),
    ("https://www.iwkoeln.de/rss.xml",                                           "IW Köln",              "eu-think"),
    ("https://www.diw.de/sixcms/detail.php?id=diw_01.c.100256.de&rss=1",         "DIW Berlin",           "eu-think"),
    ("https://www.ifo.de/rss/news.xml",                                          "ifo Institut",         "eu-think"),
    ("https://news.google.com/rss/search?q=site:stiftung-nv.de&hl=de&gl=DE&ceid=DE:de",  "Stiftung Neue Verantwortung", "eu-think"),
    ("https://news.google.com/rss/search?q=site:zew.de&hl=de&gl=DE&ceid=DE:de",  "ZEW Mannheim",         "eu-think"),
    ("https://news.google.com/rss/search?q=site:iwh-halle.de&hl=de&gl=DE&ceid=DE:de", "IWH Halle",       "eu-think"),
    ("https://news.google.com/rss/search?q=site:merics.org&hl=de&gl=DE&ceid=DE:de", "MERICS (China)",    "eu-think"),
    ("https://news.google.com/rss/search?q=site:gppi.net&hl=de&gl=DE&ceid=DE:de", "Global Public Policy Institute", "eu-think"),
...
BT_TOPIC_RULES = {
    ...
    "soziales":{"score":[(3,["rentenpaket","rentenreform","grundsicherung","bürgergeld","pflegeversicherung","kindergrundsicherung","sozialstaat","rentenpaket","rentenreform","rente","grundsicherung","bürgergeld","pflegeversicherung","kindergrundsicherung","sozialleistungen","bürgergeld","rente","krankenversicherung","pflegereform"]),(2,["sozial","arbeit","beschäftigung","mindestlohn","tarifvertrag"]),(1,["arbeitnehmer","sozialstaat","sozialpolitik"])],"min":2},
    ...
    "umwelt":{"score":[(3,["hitzewelle","waldbrand","dürre","hochwasser","artenschutz","naturschutz","luftqualität","starkregen","hitzewelle","waldbrand","dürre","hochwasser","artenschutz","naturschutz","luftqualität","klimaschutzgesetz","energiewende","emissionshandel","naturschutzgesetz","co2-preis"]),(2,["klimaschutz","umwelt","erneuerbare","solar","windkraft","co2"]),(1,["klima","energie","nachhaltig"])],"min":2},
...
US_FEEDS = [
    ...
    (_gnus("when:2d+site:apnews.com+politics"),              "AP Politics",        "us-media"),
...
NEWS_FEEDS = [
    ...
    ("https://news.google.com/rss/search?q=when:2d+site:reuters.com&ceid=US:en&hl=en-US&gl=US", "Reuters Top",      "en-top"),
    ("https://news.google.com/rss/search?q=when:2d+site:reuters.com+world&ceid=US:en&hl=en-US&gl=US", "Reuters World",   "en-top"),
    ("https://news.google.com/rss/search?q=when:2d+site:reuters.com+technology&ceid=US:en&hl=en-US&gl=US", "Reuters Tech",    "en-top"),
    ("https://news.google.com/rss/search?q=when:2d+site:reuters.com+business&ceid=US:en&hl=en-US&gl=US", "Reuters Business", "en-top"),
    # AP News: 401 → via Google News
    ("https://news.google.com/rss/search?q=when:2d+site:apnews.com&ceid=US:en&hl=en-US&gl=US", "AP News", "en-top"),
...
TOPIC_RULES = {
    ...
    # Getrennt, weil "Cyber" sonst Polizeigesetze und Extremismus mit
    # Ransomware in einen Topf wirft - das war im Interface sichtbar.
    "cyber":{"score":[(3,["cyberangriff","ransomware","malware","zero-day","exploit",
        "datenleck","kritis","ddos","phishing","schadsoftware"]),
      (2,["bsi","cybersecurity","hacker","sicherheitsluecke","sicherheitslücke","verschlüsselung",
        "it-sicherheit","botnet","cyberabwehr"]),
      (1,["backdoor","passwort","firewall"])],"min":2},
    "inneres":{"score":[(3,["innenministerium","innenministerin","innenminister","verfassungsschutz",
        "innere sicherheit","polizeigesetz","extremismus","terroranschlag","bundespolizei"]),
      (2,["bka","bnd","polizei","sabotage","spionage","razzia","terror","gefährder",
        "abschiebung","schleuser"]),
      (1,["sicherheitsbehörden","kriminalität"])],"min":2},
    "energie":{"score":[(3,["stromtrasse","stromnetz","netzagentur","netzentgelt","strompreis","gaspreis","kraftwerk","energiewende","erneuerbare energien","energiesicherheit","atomkraft","kernkraft","lng"]),(2,["solar","windkraft","wasserstoff","strompreis","gaspreise","co2","klimaschutz","kraftwerk"]),(1,["energie","strom","gas ","öl ","pipeline"])],"min":2},
    ...
    # Schwelle 5: Eine einzelne Erwaehnung ("KI-Chips nach China") macht aus einer
    # Technikmeldung noch keine China-Meldung. Es braucht zwei starke Begriffe
    # oder einen starken plus zwei schwache. Firmennamen zaehlen nur schwach,
    # weil Huawei in Ungarn baut und TSMC in Dresden.
    "china":{"score":[(3,["china","chinas","peking","beijing","xi jinping","taiwan","hongkong",
                          "volksrepublik","kommunistische partei chinas","südchinesisches meer",
                          "seidenstraße","belt and road","taiwanstraße"]),
                      (2,["chinesisch","chinesische","chinesischen","yuan","renminbi","xinjiang",
                          "tibet","shanghai","shenzhen","handelsstreit mit china"]),
                      (1,["tsmc","huawei","alibaba","byd","cosco","asien-pazifik","indo-pazifik"])],
             "min":4},   # zwei starke Begriffe (3 + 1,5) reichen, einer nicht
    "asien":{"score":[(3,["japan","südkorea","nordkorea","indien","indonesien","vietnam","philippinen"]),(2,["asean","tokio","neu-delhi","seoul","pjöngjang"]),(1,["asien","pazifik","indo-pazifik"])],"min":2},
    ...
    "wissenschaft":{"score":[(3,["studie","forschende","forschungsprojekt","universität","institut","fachzeitschrift","peer-review","antibiotika","peer-reviewed","forschungsergebnis","quantencomputer","crispr","durchbruch"]),(2,["universität","forschung","physik","biologie","genetik","fraunhofer","max planck"]),(1,["wissenschaft","labor","theorie","entdeckung"])],"min":2},
    ...
    "soziales":{"score":[(3,["rentenpaket","rentenreform","grundsicherung","bürgergeld","pflegeversicherung","kindergrundsicherung","bürgergeld","rente","rentenreform","mindestlohn","sozialstaat","grundsicherung"]),(2,["sozial","arbeitslos","armut","kindergeld","elterngeld","tarif"]),(1,["gewerkschaft","sozialverband"])],"min":2},
    ...
    "migration":{"score":[(3,["asylrecht","asylverfahren","asylpolitik","geflüchtete","asylpolitik","migrationspolitik","abschiebung","grenzkontrolle","frontex","asylreform"]),(2,["migration","asyl","flüchtlinge","einwanderung","integration"]),(1,["zuwanderung"])],"min":2},
    ...
    "umwelt":{"score":[(3,["hitzewelle","waldbrand","dürre","hochwasser","artenschutz","naturschutz","starkregen","umweltministerium","naturschutz","artenschutz","klimaanpassung","hochwasser","dürre"]),(2,["umwelt","biodiversität","wald","gewässer","plastik","müll"]),(1,["ökologisch"])],"min":2},
    ...
    "kultur":{"score":[(3,["berlinale","filmfest","festival","museum","theater","oper","buchmesse","kulturstaatsminister","kulturstaatsminister","rundfunkbeitrag","öffentlich-rechtlich","medienstaatsvertrag","pressefreiheit"]),(2,["kultur","medien","rundfunk","verlag","museum","theater"]),(1,["journalismus"])],"min":2},
...
TOPIC_MERGE = {
    ...
    "innenpolitik":"inneres", "sicherheitspolitik":"verteidigung",
    "verbraucher":"wirtschaft", "handel":"wirtschaft", "startup":"wirtschaft",
    "raumfahrt":"wissenschaft",
    "religion":"politik", "kommunales":"politik",
    # "china" bleibt eigenstaendig (Regionsleiste), alles uebrige Internationale
    # laeuft unter "international" – im Interface als "Welt" beschriftet.
...
def score_article(text, rules):
    """Punkte je Thema. Mehrere Treffer derselben Stufe zaehlen mit, aber
    abgeschwaecht und gedeckelt.

    Vorher brach die Schleife nach dem ersten Treffer je Stufe ab ("break").
    Damit war "China" genauso stark wie "China ... Peking ... Xi Jinping" -
    eine Technikmeldung, die China einmal streift, bekam dieselbe Punktzahl wie
    ein echter China-Bericht. Jetzt zaehlt der erste Treffer voll, jeder weitere
    derselben Stufe zur Haelfte, hoechstens jedoch das Doppelte der Stufe."""
    t=text.lower()
    result={}
    for topic,rule in rules.items():
        ...
        for pts,kws in rule["score"]:
            ...
            for kw in kws:
                if kw_hit(kw, t):
                    treffer+=1
                    if treffer>=4: break      # mehr bringt nichts
            if treffer:
                s+=min(pts*2, pts + (treffer-1)*pts*0.5)
        if s>=rule["min"]:
            result[topic]=round(s,1)
    return result
...
def parse_feed(fetch_result, source, topic_rules, cat=""):
    ...
    for item in items:
        ...
        access = detect_access_cat(source, title, cats, link, cat, desc)
        if access == "paid":
...
# Formulierungen, mit denen Verlage einen Artikel als kostenpflichtig
# kennzeichnen - im Titel, in der Kategorie oder im Vorschautext.
PAID_TEXT_RE = re.compile(
    r"("
    # deutsch
    r"nur f(ü|ue)r abonnenten|exklusiv f(ü|ue)r abonnenten|abo-?artikel|plus-?artikel|"
    r"premium-?artikel|kostenpflichtiger inhalt|hinter der bezahlschranke|"
    r"jetzt weiterlesen mit|weiterlesen mit .{0,12}plus|mit .{0,10}plus weiterlesen|"
    r"abonnement erforderlich|nur mit abo"
    # englisch - die grossen Haeuser kennzeichnen unterschiedlich
    r"|subscribers? only|for subscribers|subscriber exclusive|paid article|premium content|"
    r"this article is for|sign in to read|log in to continue|register to continue|"
    r"unlock this article|behind the paywall|paywalled|"
    r"subscribe to (read|continue)|become a (member|subscriber) to"
    r")", re.I)

def detect_access(source, title, cats, link="", desc=""):
    """'paid', 'free' oder 'unknown' - bewusst zurueckhaltend.

    Lieber 'unknown' als eine falsche Behauptung: Wer sich auf 'frei' verlaesst
    und dann vor einer Bezahlschranke steht, aergert sich mehr, als wenn gar
    nichts dasteht.
    """
    if source in PAID_SOURCES: return "paid"
    if any(re.fullmatch(r"(s\+|plus|abo|premium|paywall|paid)", str(c).strip(), re.I) for c in (cats or [])):
        return "paid"
    if PAID_MARK_RE.search(title or ""): return "paid"
    # Viele Feeds schreiben den Hinweis in den Vorschautext statt in den Titel.
    if PAID_TEXT_RE.search(title or "") or PAID_TEXT_RE.search(desc or ""): return "paid"
    l = (link or "").lower()
    if re.search(r"[/\-.](plus|premium|abo|paywall|paid)[/\-.?]|[/\-]plus\b", l): return "paid"
    # Ein abgeschnittener Vorschautext mit Weiterlesen-Aufforderung ist ein
    # zuverlaessiges Zeichen fuer eine Bezahlschranke.
    if re.search(r"(weiterlesen|read more|continue reading)\s*[.…»>]*\s*$", (desc or "").strip(), re.I):
        return "paid"
    if source in FREE_SOURCES: return "free"
    return "unknown"

def detect_access_cat(source, title, cats, link="", cat="", desc=""):
    """Wie detect_access, beruecksichtigt zusaetzlich die Feed-Kategorie."""
    a = detect_access(source, title, cats, link, desc)
...
# Viele Behoerden- und Zeitungsseiten (whitehouse.gov, congress.gov, WaPo,
# cisa.gov, nature.com) sperren Bot-Kennungen mit HTTP 403. Deshalb eine
# gewoehnliche Browser-Kennung, und bei Fehlschlag ein zweiter Versuch mit einer
# anderen. Das ist kein Trick, sondern schlicht die Kennung, die diese Seiten
# erwarten - abgerufen werden oeffentliche RSS-Feeds.
UA_LISTE = [
...
def _hole(url_encoded, timeout, ua):
    headers = {
        "User-Agent": ua,
...
def fetch_url(url, timeout=int(os.environ.get("FEED_TIMEOUT", "10"))):
    ...
    letzter = None
    for versuch, ua in enumerate(UA_LISTE):
        try:
            return _hole(url_encoded, timeout, ua)
        except Exception as e:
            letzter = e
            # Bei 403/429 lohnt ein zweiter Versuch mit anderer Kennung,
            # bei 404 oder Netzfehlern nicht.
            code = getattr(e, "code", None)
            if code not in (403, 429, None) or versuch == len(UA_LISTE) - 1:
                break
    print(f"ERR {type(letzter).__name__}", end=" ")
    return None

def google_fallback(url, name):
    """Liefert eine Google-News-Suche fuer die Domain eines gescheiterten Feeds.

    Manche Seiten bieten gar keinen (funktionierenden) Feed mehr an. Statt die
    Quelle zu verlieren, wird ueber Google News nach Meldungen dieser Domain
    gesucht. Das liefert weniger Volltext, aber die Schlagzeilen bleiben da.
    """
    from urllib.parse import urlparse
    host = (urlparse(url).netloc or "").replace("www.", "")
    if not host or "news.google.com" in url:
        return None
    return ("https://news.google.com/rss/search?q=site:" + host +
            "+when:2d&hl=de&gl=DE&ceid=DE:de")
...
def fetch_all(feed_list, topic_rules, label):
    ...
    # Zweiter Durchgang: was nicht geantwortet hat, ueber Google News nachholen.
    # Viele Seiten haben ihren Feed abgeschaltet oder sperren Abrufe; die
    # Schlagzeilen sind aber ueber die Nachrichtensuche weiter erreichbar.
    if os.environ.get("FEED_FALLBACK", "1") not in ("0", "false", "no"):
        offen = [(u, n, c) for u, n, c in feed_list if not roh.get(n)]
        if offen and not out_of_time():
            print(f"  [{label}] {len(offen)} Feeds ohne Antwort – Versuch über Nachrichtensuche")
            with ThreadPoolExecutor(max_workers=FEED_WORKERS) as pool2:
                auftraege2 = {}
                for url, name, cat in offen:
                    alt = google_fallback(url, name)
                    if alt: auftraege2[pool2.submit(fetch_url, alt)] = name
                for fut, name in auftraege2.items():
                    if out_of_time(): fut.cancel(); continue
                    try:
                        r = fut.result(timeout=max(1, int(_DEADLINE - time.monotonic())))
                        if r: roh[name] = r
                    except Exception:
                        pass
...
# ─────────────────────────────────────────────────────────────
# ENTITAETEN (spaCy, optional)
#
# Die bisherige Begriffserkennung im Browser arbeitet rein nach Grossschreibung.
# Im Deutschen ist jedes Substantiv gross, deshalb landen "Gesetzentwurf" und
# "Kabinett" gleichberechtigt neben "Lagarde" und "Sachsen-Anhalt".
# spaCy erkennt stattdessen echte Eigennamen (Personen, Organisationen, Orte).
#
# Bewusst als KANN-Funktion gebaut: ist spaCy nicht installiert, laeuft alles
# unveraendert weiter. Damit bleibt der Abruf lauffaehig, auch wenn die
# Installation im Workflow einmal fehlschlaegt.
#
# Gemessen: rund 900 Texte je Sekunde, 5.000 Artikel in etwa 6 Sekunden.
# Die Labels des kleinen Modells sind bei kurzen Schlagzeilen unzuverlaessig
# ("Merz" wird als Ort gefuehrt) - deshalb wird nur der TEXT der Entitaet
# verwendet, nicht ihre Einstufung.
_NLP = None
_NLP_TRIED = False
...
def _nlp():
    ...
    _NLP_TRIED = True
    if os.environ.get("SPACY", "1") in ("0", "false", "no"):
        print("── Entitäten: per SPACY=0 abgeschaltet ──"); return None
    try:
        import spacy
        # Tagger und Lemmatizer werden fuer die Wortarten gebraucht: Nur so
        # laesst sich "beschliesst" (Verb) von "Verwaltung" (Substantiv) trennen.
        # Kostet rund 3 Sekunden je 5.000 Titel - vertretbar.
        # Tagger und Lemmatizer bleiben aktiv: Sie unterscheiden Substantive von Verben
        # und Adjektiven. Ohne sie landeten "beschließt" (Verb) und "digitalen"
        # (Adjektiv) in der Themenauswertung. Nur der Parser entfällt.
        _NLP = spacy.load("de_core_news_sm")
        print("── Entitäten: spaCy de_core_news_sm geladen ──")
    except Exception as e:
        print(f"── Entitäten: spaCy nicht verfügbar ({type(e).__name__}) – "
              f"Begriffe werden wie bisher im Browser bestimmt ──")
        _NLP = None
    return _NLP

# Woerter, die spaCy haeufig faelschlich als Eigennamen fuehrt und die inhaltlich
# nichts aussagen (Verfahrensvokabular).
ENT_STOP = {
    "gesetzentwurf", "gesetz", "entwurf", "reform", "antrag", "anfrage", "bericht",
    "drucksache", "sitzung", "tagesordnung", "beschluss", "verordnung", "richtlinie",
    "kabinett", "anhörung", "debatte", "rede", "stellungnahme", "mitteilung",
    "montag", "dienstag", "mittwoch", "donnerstag", "freitag", "samstag", "sonntag",
    "januar", "februar", "märz", "april", "mai", "juni", "juli", "august",
    "september", "oktober", "november", "dezember",
}

# Substantive, die formal Substantive sind, aber inhaltlich nichts aussagen.
# Sie tauchen in jeder zweiten Schlagzeile auf und wuerden die Trendliste fuellen.
TERM_STOP = {
    "kraft", "rahmen", "hinblick", "bezug", "zusammenhang", "folge", "zukunft",
    "frage", "thema", "punkt", "entscheidung", "beschluss", "vorschlag", "plan",
    "ziel", "streit", "debatte", "diskussion", "bericht", "interview", "analyse",
    "kommentar", "meinung", "video", "podcast", "newsletter", "jahr", "monat",
    "woche", "tag", "zeit", "prozent", "million", "milliarde", "euro", "dollar",
    "mensch", "land", "stadt", "welt", "teil", "seite", "ende", "anfang",
    "deutschland", "europa", "bundestag", "bundesrat", "bundesregierung",
    "regierung", "koalition", "opposition", "parlament", "kommission",
    "ausschuss", "ministerium", "minister", "kanzler", "politik", "politiker",
}
...
def _nlp_en():
    """Englisches Modell – optional. Fehlt es, werden englische Texte mit dem
    deutschen Modell verarbeitet; das erkennt Eigennamen noch brauchbar, die
    Wortarten aber nicht."""
    global _NLP_EN, _NLP_EN_TRIED
    if _NLP_EN_TRIED: return _NLP_EN
    _NLP_EN_TRIED = True
    try:
        import spacy
        _NLP_EN = spacy.load("en_core_web_sm")
        print("── Begriffe: englisches Modell geladen ──")
    except Exception:
        _NLP_EN = None
    return _NLP_EN

# Sprachbestimmung ohne Zusatzbibliothek: ein paar Funktionswoerter genuegen,
# weil sie in jedem Satz vorkommen und sich zwischen den Sprachen nicht
# ueberschneiden.
_DE_MARKER = re.compile(r'\b(der|die|das|und|nicht|werden|wird|für|über|nach|bei|mit|sich|eine|einen)\b', re.I)
_EN_MARKER = re.compile(r'\b(the|and|with|after|from|that|will|says|been|about|would|their)\b', re.I)

def _sprache(text):
    de = len(_DE_MARKER.findall(text or ""))
    en = len(_EN_MARKER.findall(text or ""))
    return "en" if en > de else "de"

def add_terms(articles):
    """Schreibt je Artikel die inhaltstragenden Substantive nach a['terms'].

    Warum ueber die Wortart und nicht ueber Endungen oder Grossschreibung:
    Im Deutschen sind alle Substantive gross, im Englischen nur Eigennamen -
    jede Regel ueber die Schreibung scheitert an einer der beiden Sprachen.
    "beschliesst" ist gross am Satzanfang, "digitalen" ist ein Adjektiv, beide
    landeten frueher in der Trendliste. spaCy bestimmt die Wortart direkt.

    Zusaetzlich die Grundform (Lemma): "Reformen" und "Reform" zaehlen damit
    zusammen, ohne dass Stammformregeln je Sprache noetig waeren.
    """
    nlp = _nlp()
    if not nlp or not articles: return
    t0 = time.time()
    # Nur Nachrichten: Parlamentstitel ("Tagesordnung der 42. Sitzung") bestehen
    # aus Verfahrensvokabular und wuerden die Auswertung zumuellen.
    NACHRICHTEN = {"news", "eu", "us"}
    kandidaten = [a for a in articles if a.get("_set", "news") in NACHRICHTEN]
    for a in articles:
        if a not in kandidaten: a.pop("terms", None)
    if not kandidaten:
        print("  Begriffe: keine Nachrichtenartikel"); return
    # Der Teaser enthaelt oft erst die Sachfrage – deshalb Titel UND Vorschautext.
    texte_alle = [((a.get("title") or "") + ". " + (a.get("desc") or ""))[:320] for a in kandidaten]
    # Nach Sprache trennen, damit jedes Modell das bekommt, wofuer es trainiert ist
    idx_de = [i for i, t in enumerate(texte_alle) if _sprache(t) == "de"]
    idx_en = [i for i, t in enumerate(texte_alle) if _sprache(t) == "en"]
    nlp_en = _nlp_en() if idx_en else None
    paare = []
    if idx_de:
        paare += list(zip([kandidaten[i] for i in idx_de],
                          nlp.pipe([texte_alle[i] for i in idx_de], batch_size=64)))
    if idx_en:
        motor = nlp_en or nlp
        paare += list(zip([kandidaten[i] for i in idx_en],
                          motor.pipe([texte_alle[i] for i in idx_en], batch_size=64)))
    articles, texte = kandidaten, texte_alle
    gesamt = 0
    try:
        for a, doc in paare:
            terms = []
            for chunk in doc.noun_chunks:
                # Artikel und Pronomen am Anfang weg ("die Anhebung" -> "Anhebung",
                # "the AI Act" -> "AI Act"). Adjektive bleiben: "digitale
                # Verwaltung" sagt mehr als "Verwaltung".
                toks = [t for t in chunk
                        if t.pos_ not in ("DET", "PRON") and not t.is_punct]
                while toks and toks[0].is_stop: toks.pop(0)
                if not toks: continue
                kopf = toks[-1]
                if kopf.pos_ not in ("NOUN", "PROPN"): continue
                kopf_lem = (kopf.lemma_ or kopf.text).lower()
                if kopf_lem in TERM_STOP: continue          # "Kabinett", "Senat" …
                # Phrase zusammensetzen: vordere Teile im Original, Kopf in Grundform,
                # damit "des Mindestlohns" und "der Mindestlohn" zusammenfallen.
                teile = [t.text for t in toks[:-1]] + [(kopf.lemma_ or kopf.text)]
                # Originalschreibweise bewahren: "digitale Verwaltung" statt
                # "digitalen verwaltung". Die Oberflaeche vergleicht in
                # Kleinschreibung, zeigt aber diese Form an.
                phrase = " ".join(teile).strip(" -–.,;:")
                if len(phrase) < 6: continue
                if re.fullmatch(r"[\d\s.,%%-]+", phrase): continue   # reine Zahlen
                if len(toks) == 1 and len(phrase) < 10: continue
                if phrase.lower() not in [x.lower() for x in terms]:
                    terms.append(phrase)
            a["terms"] = terms[:8]
            gesamt += len(terms)
    except Exception as e:
        print(f"  Begriffe uebersprungen: {type(e).__name__}: {e}")
        return
    print(f"  {gesamt} Begriffe in {len(articles)} Nachrichten "
          f"({len(idx_de)} de / {len(idx_en)} en, {time.time()-t0:.1f}s)")

def add_entities(articles, limit_chars=400):
    """Schreibt je Artikel eine Liste erkannter Eigennamen nach a['ents']."""
    nlp = _nlp()
    if not nlp or not articles: return
    t0 = time.time()
    texte = [((a.get("title") or "") + ". " + (a.get("desc") or ""))[:limit_chars] for a in articles]
    treffer = 0
    try:
        for a, doc in zip(articles, nlp.pipe(texte, batch_size=64)):
            ents = []
            for e in doc.ents:
                w = e.text.strip(" .,;:–-")
                if len(w) < 4 or w.lower() in ENT_STOP: continue
                if w.isdigit(): continue
                if w not in ents: ents.append(w)
            a["ents"] = ents[:8]
            treffer += len(ents)
    except Exception as e:
        print(f"  Entitäten übersprungen: {type(e).__name__}: {e}")
        return
    print(f"  {treffer} Eigennamen in {len(articles)} Artikeln ({time.time()-t0:.1f}s)")

# ─────────────────────────────────────────────────────────────
# TONLAGE (regelbasiert, nachvollziehbar)
#
# Bewusst KEINE Stimmungsanalyse im ueblichen Sinn. Gruende:
#  · TextBlob & Co. arbeiten nur auf Englisch (auf deutschen Schlagzeilen
#    liefern sie durchgaengig 0,00 - nachgemessen).
#  · Deutsche BERT-Modelle koennen es, brauchen aber ~450 MB und Rechenzeit.
#  · Vor allem: eine Zahl wie "-0,7" ist bei politischer Berichterstattung
#    irrefuehrend. "Opposition kritisiert Haushalt" ist sachliche Berichter-
#    stattung, kein negativer Ton. Entscheidungstraeger wuerden aus so einer
#    Zahl falsche Schluesse ziehen.
#
# Stattdessen: wenige klar benannte Signale, die zeigen, WORAN sie erkannt
# wurden. Nachvollziehbar statt Blackbox.
TON_MARKER = {
    "streit":     ["streit", "konflikt", "eklat", "zerwürfnis", "blockade", "widerstand",
                   "auseinandersetzung", "machtkampf", "koalitionskrach", "zoff"],
    "kritik":     ["kritisiert", "kritik", "vorwurf", "vorwürfe", "rüge", "wirft vor",
                   "bemängelt", "beanstandet", "weist zurück", "empört", "scharf"],
    "zustimmung": ["begrüßt", "zustimmung", "einigung", "kompromiss", "durchbruch",
                   "beschlossen", "gebilligt", "verabschiedet", "angenommen", "einig"],
    "warnung":    ["warnt", "warnung", "mahnt", "befürchtet", "alarm", "risiko",
                   "gefahr", "droht", "bedenken"],
}

# Sendungsfolgen sind keine Nachrichten: "ZDF-Mittagsmagazin vom 10. September",
# "logo! vom Freitag", "heute journal", "Tagesschau um 20 Uhr". Sie erscheinen
# taeglich, tragen keine eigene Aussage im Titel und verdraengen echte Meldungen
# aus den Top-Meldungen. Erkennbar am Muster "<Sendung> vom <Datum>" oder an
# bekannten Sendungsnamen.
SENDUNG_RE = re.compile(
    r'^(logo!?|heute[- ]journal|heute in|tagesschau|tagesthemen|mittagsmagazin|morgenmagazin|'
    r'nachtmagazin|zdf[- ]?(heute|mittagsmagazin|morgenmagazin)|wdr aktuell|br24 (am|um)|'
    r'sendung vom|die nachrichten|nachrichten vom)\b'
    r'|(mittagsmagazin|morgenmagazin|heute[- ]journal|tagesschau|tagesthemen|logo!)\s+vom\b'
    r'|\bvom\s+(montag|dienstag|mittwoch|donnerstag|freitag|samstag|sonntag)\b',
    re.I)

def ist_sendung(titel):
    return bool(SENDUNG_RE.search(titel or ""))

def add_tone(articles):
    """Setzt a['ton'] = {'art': str, 'woerter': [...]} – nur wenn eindeutig."""
    for a in articles:
        text = ((a.get("title") or "") + " " + (a.get("desc") or "")).lower()
        treffer = {}
        for art, woerter in TON_MARKER.items():
            gefunden = [w for w in woerter if w in text]
            if gefunden: treffer[art] = gefunden
        if not treffer:
            a.pop("ton", None); continue
        # Nur auszeichnen, wenn EINE Kategorie klar ueberwiegt - sonst lieber nichts
        beste = max(treffer.items(), key=lambda x: len(x[1]))
        if len([k for k, v in treffer.items() if len(v) == len(beste[1])]) > 1:
            a.pop("ton", None); continue
        a["ton"] = {"art": beste[0], "woerter": beste[1][:3]}

def verfeinere_zugang(articles, pfad="zugang_report.md"):
    """Bestimmt den Zugang JE ARTIKEL, nicht je Quelle.

    Eine Quelle pauschal als "Abo" einzustufen ist falsch: Der Spiegel hat
    freie Meldungen und S+-Artikel, ebenso Zeit, FAZ oder Welt. Entscheidend
    ist deshalb nicht, wie oft eine Quelle kostenpflichtig ist, sondern ob sie
    ihre kostenpflichtigen Artikel ERKENNBAR MACHT.

    Der Schluss, auf den es ankommt:
      Kennzeichnet eine Quelle einen Teil ihrer Artikel ausdruecklich als
      kostenpflichtig (S+, Plus-Link, "nur fuer Abonnenten"), dann ist das
      Fehlen dieser Kennzeichnung bei den uebrigen Artikeln eine Aussage -
      sie sind frei. Wer seine Abo-Artikel markiert, markiert sie konsequent.

    Quellen, die NIE kennzeichnen, bleiben unbestimmt. Dort waere jede
    Behauptung geraten, und "unbekannt" ist ehrlicher als eine falsche Angabe.
    """
    from collections import defaultdict
    proQuelle = defaultdict(lambda: {"markiert": 0, "ohne": 0, "n": 0})
    for a in articles:
        z = proQuelle[a.get("source") or "?"]
        z["n"] += 1
        if a.get("access") == "paid": z["markiert"] += 1
        elif a.get("access") == "unknown": z["ohne"] += 1

    # Eine Quelle "kennzeichnet zuverlaessig", wenn sie genug Artikel hat und
    # einen erkennbaren Teil davon markiert - aber nicht alles (sonst koennte
    # es auch eine reine Abo-Quelle sein, bei der nichts zu schliessen ist).
    kennzeichnet = {q for q, z in proQuelle.items()
                    if z["n"] >= 6 and z["markiert"] >= 2 and z["ohne"] >= 2}

    geschlossen = 0
    for a in articles:
        if a.get("access") == "unknown" and (a.get("source") or "?") in kennzeichnet:
            a["access"] = "free"
            a["access_grund"] = "abgeleitet"   # nachvollziehbar, woher die Angabe stammt
            geschlossen += 1

    stumm = sorted(((q, z["n"]) for q, z in proQuelle.items()
                    if z["n"] >= 6 and z["markiert"] == 0 and q not in FREE_SOURCES
                    and q not in PAID_SOURCES), key=lambda x: -x[1])

    zeilen = [f"# Zugangserkennung - Stand {datetime.now(timezone.utc):%d.%m.%Y %H:%M} UTC", "",
              f"{len(proQuelle)} Quellen, {len(articles)} Meldungen.", "",
              "Die Einstufung erfolgt **je Artikel**, nicht je Quelle - Häuser wie Spiegel",
              "oder Zeit haben beides. Wer seine Abo-Artikel kennzeichnet, macht damit auch",
              "eine Aussage über die übrigen: Sie sind frei.", "",
              f"## Quellen mit zuverlässiger Kennzeichnung ({len(kennzeichnet)})", "",
              f"Bei ihnen wurden {geschlossen} Meldungen ohne Kennzeichen als *frei* eingestuft.", ""]
    for q in sorted(kennzeichnet):
        z = proQuelle[q]
        zeilen.append(f"- {q}: {z['markiert']} von {z['n']} gekennzeichnet")
    zeilen += ["", f"## Quellen ohne jede Kennzeichnung ({len(stumm)})", "",
               "Hier lässt sich nichts ableiten - sie bleiben unbestimmt, es wird also",
               "nichts angezeigt. Wenn du eine davon kennst, gehört sie in `PAID_SOURCES`",
               "oder `FREE_SOURCES` in fetch_news.py.", ""]
    zeilen += [f"- {q} ({n} Meldungen)" for q, n in stumm[:80]]
    try:
        with open(pfad, "w", encoding="utf-8") as f:
            f.write("\n".join(zeilen) + "\n")
        print(f"  Zugang: {len(kennzeichnet)} Quellen kennzeichnen zuverlässig, "
              f"{geschlossen} Meldungen abgeleitet, {len(stumm)} Quellen unbestimmt")
    except Exception as e:
        print(f"  Zugangsbericht übersprungen: {type(e).__name__}")
...
# ═══════════════════════════════════════════════════════════════
# ANREICHERUNG (v10) – herkunft, Gerichtsangaben, Personen
# ═══════════════════════════════════════════════════════════════
# Vier Angaben, die das Frontend braucht und die bisher fehlten:
#   herkunft  medien | parlament | eu-inst | laender | gericht | think
#             → Filter "Alle Herkünfte", Farbe der Kacheln, Trennung von
#               "Studien & Analysen" und Nachrichten
#   gericht / az / ls   Gericht, Aktenzeichen, Leitsatz (Ansicht "Urteile")
#   personen  welche Abgeordneten in der Meldung vorkommen, aus people.json
#             und den schon erkannten Eigennamen (a["ents"])
# Alle bisherigen Felder bleiben unverändert; die neuen kommen additiv dazu.
# Aufgerufen wird das Ganze an einer Stelle: in save_json().

# ═══════════════════════════════════════════════════════════════
# 1. HERKUNFT
# ═══════════════════════════════════════════════════════════════
# Reihenfolge der Prüfung: Gericht schlägt Institution schlägt Land
# schlägt Think Tank schlägt Medien.

GERICHT_CATS = {"cjeu", "bverfg", "bgh", "bverwg", "bag", "bsg", "bfh", "eug", "us-scotus"}
EU_CATS      = {"ep", "ep-committee", "ec", "council", "eu-council", "ecb", "eca", "eu-inst", "eurlex"}
PARL_CATS    = {"bt-allg", "bt-thema", "breg", "bundesrat",
                "us-house", "us-senate", "us-congress", "us-whitehouse", "us-agency"}
THINK_CATS   = {"eu-think", "think"}

# Länderkürzel kommen aus LAENDER weiter oben – eine Liste, eine Wahrheit.
LAND_CODES = {c for c, _, _ in LAENDER}


def herkunft(source: str, cat: str, inst: str = "") -> str:
    c = (cat or "").lower()
    s = (source or "").lower()
    i = (inst or "").lower()
    if c in GERICHT_CATS or i in GERICHT_CATS:
        return "gericht"
    if c in THINK_CATS:
        return "think"
    if c in EU_CATS or i in {"ep", "ec", "eu-council", "ecb", "eca", "eurlex", "eu-agency"}:
        return "eu-inst"
    if c in LAND_CODES or i.startswith(("landtag-", "lreg-")):
        return "laender"
    if c in PARL_CATS or i in {"bundestag", "bundesrat", "bundesregierung"}:
        return "parlament"
    # Notnagel, falls eine Quelle ohne cat durchrutscht
    if any(w in s for w in ("bundestag", "bundesrat", "bundesregierung", "white house", "congress")):
        return "parlament"
    return "medien"


# ═══════════════════════════════════════════════════════════════
# 2. URTEILE: Gericht, Aktenzeichen, Leitsatz
# ═══════════════════════════════════════════════════════════════
GERICHT_NAME = {
    "cjeu": "EuGH", "eug": "EuG", "bverfg": "BVerfG", "bgh": "BGH",
    "bverwg": "BVerwG", "bag": "BAG", "bsg": "BSG", "bfh": "BFH",
    "us-scotus": "Supreme Court",
}

# Aktenzeichen der einzelnen Gerichte. Reihenfolge: das spezifischste zuerst,
# sonst frisst das BGH-Muster die Nummern der anderen mit.
AZ_MUSTER = [
    ("cjeu",   re.compile(r"\b(?:Rs\.\s*)?(C-\d{1,4}/\d{2})\b")),                 # C-412/24
    ("eug",    re.compile(r"\b(T-\d{1,4}/\d{2})\b")),                             # T-58/25
    ("bverfg", re.compile(r"\b(\d\s?Bv[A-Z]\s?\d{1,5}/\d{2})\b")),                # 2 BvR 1188/26
    ("bverwg", re.compile(r"\b(\d{1,2}\s?[A-Z]{1,2}\s?\d{1,3}\.\d{2})\b")),       # 4 C 3.25
    ("bgh",    re.compile(r"\b([IVX]{1,4}\s?[A-Z]{1,3}\s?\d{1,4}/\d{2})\b")),     # I ZR 77/25
    ("bag",    re.compile(r"\b(\d{1,2}\s?AZR\s?\d{1,4}/\d{2})\b")),
    ("bsg",    re.compile(r"\b(B\s?\d{1,2}\s?[A-Z]{1,3}\s?\d{1,3}/\d{2}\s?[A-Z]?)\b")),
    ("bfh",    re.compile(r"\b([IVX]{1,4}\s?R\s?\d{1,3}/\d{2})\b")),
    ("us-scotus", re.compile(r"\bNo\.\s?(\d{2}-\d{2,5})\b")),
]

# Leitsatz: viele Pressemitteilungen schreiben ihn aus. Wenn nicht, nimmt die
# Funktion den ersten vollständigen Satz des Textes – das ist bei Gerichten
# fast immer der Tenor.
LS_MARKER = re.compile(
    r"(?:Leitsatz|Leitsätze|Tenor|Entscheidungsformel)\s*[:\-–]\s*(.+)", re.I | re.S)
SATZ_ENDE = re.compile(r"(?<=[.!?])\s+(?=[A-ZÄÖÜ])")


def _erster_satz(text: str, max_len: int = 320) -> str:
    text = (text or "").strip()
    if not text:
        return ""
    teile = SATZ_ENDE.split(text)
    satz = teile[0].strip()
    # Ein Ein-Wort-Fragment ("Urteil.") sagt nichts – dann den nächsten Satz dazu.
    if len(satz) < 60 and len(teile) > 1:
        satz = (satz + " " + teile[1].strip()).strip()
    return satz[:max_len].rstrip(" ,;–-")


def gericht_info(artikel: dict) -> dict:
    """Gibt {"gericht":…, "az":…, "ls":…} zurück – leere Werte fallen weg."""
    cat = (artikel.get("cat") or "").lower()
    inst = (artikel.get("inst") or "").lower()
    quelle = artikel.get("source") or ""
    schluessel = cat if cat in GERICHT_NAME else (inst if inst in GERICHT_NAME else "")
    if not schluessel:
        return {}

    text = (artikel.get("title") or "") + " " + (artikel.get("desc") or "")
    az = ""
    # Erst das Muster des eigenen Gerichts, dann alle übrigen.
    reihenfolge = ([p for p in AZ_MUSTER if p[0] == schluessel] +
                   [p for p in AZ_MUSTER if p[0] != schluessel])
    for _, rx in reihenfolge:
        m = rx.search(text)
        if m:
            az = re.sub(r"\s+", " ", m.group(1)).strip()
            break

    m = LS_MARKER.search(artikel.get("desc") or "")
    ls = _erster_satz(m.group(1)) if m else _erster_satz(artikel.get("desc") or "")

    out = {"gericht": GERICHT_NAME.get(schluessel, quelle)}
    if az:
        out["az"] = az
    if ls:
        out["ls"] = ls
...
# ═══════════════════════════════════════════════════════════════
# 3. GERICHTS-FEEDS, DIE BISHER FEHLTEN
# ═══════════════════════════════════════════════════════════════
# Format wie oben: (url, name, cat) – _gn() ist weiter oben definiert.
# EuGH (cjeu), BGH und BVerfG stehen dort schon; hier kommen die übrigen dazu.
GERICHTS_FEEDS = [
    # Bundesverwaltungsgericht – eigener Pressefeed
    ("https://www.bverwg.de/rss/pressemitteilungen.xml", "BVerwG", "bverwg"),
    # Bundesarbeits-, Bundessozial- und Bundesfinanzgericht: kein stabiler RSS,
    # deshalb über die Nachrichtensuche – wie bei SWP und Bruegel auch.
    (_gn("site:bundesarbeitsgericht.de"), "BAG", "bag"),
    (_gn("site:bsg.bund.de"), "BSG", "bsg"),
    (_gn("site:bundesfinanzhof.de"), "BFH", "bfh"),
    # Gericht der EU (erste Instanz, Kartell- und Beihilfesachen)
    (_gn("site:curia.europa.eu+Gericht"), "EuG", "eug"),
]


# ═══════════════════════════════════════════════════════════════
# 4. PERSONEN IN MELDUNGEN
# ═══════════════════════════════════════════════════════════════
# people.json liefert das Verzeichnis (Abgeordnete, Ausschussmitglieder).
# Erkannt wird über die schon vorhandenen Eigennamen in a["ents"]; nur wenn
# die fehlen, wird im Titel nachgesehen. Der Nachname allein genügt nicht –
# "Weber" und "Schmidt" gibt es zu oft. Verlangt wird der volle Name oder
# Nachname plus Partei/Rolle im selben Text.

_PERSONEN_CACHE = None


def lade_personen(pfad: str = "people.json") -> list:
    """Verzeichnis einmal je Lauf lesen – save_json() ruft das fünfmal auf."""
    global _PERSONEN_CACHE
    ...
    try:
        ...
        _PERSONEN_CACHE = []
        return _PERSONEN_CACHE
    leute = roh.get("people", roh) if isinstance(roh, dict) else roh
    out = []
    for p in leute or []:
        ...
        out.append({
            ...
            "name": name,
            "nach": nach,
            "partei": (p.get("party") or p.get("partei") or "").lower(),
        })
    _PERSONEN_CACHE = out
    return out
...
# ═══════════════════════════════════════════════════════════════
# ZENTRALE FUNKTION – die eine Zeile in save_json()
# ═══════════════════════════════════════════════════════════════
def anreichern(a: dict, personen: list = None) -> dict:
    a["herkunft"] = herkunft(a.get("source"), a.get("cat"), a.get("inst"))
    if a["herkunft"] == "gericht":
        a.update(gericht_info(a))
    if personen:
        p = personen_in(a, personen)
        if p:
            a["personen"] = p
    return a
...
def save_json(filename, articles, ok, fail, extra=None):
    ...
    try: add_entities(deduped)
    except Exception as e: print(f"  add_entities: {type(e).__name__}: {e}")
    try: verfeinere_zugang(deduped)
    except Exception as e: print(f"  verfeinere_zugang: {type(e).__name__}: {e}")
    try: add_terms(deduped)
    except Exception as e: print(f"  add_terms: {type(e).__name__}: {e}")
    try: add_tone(deduped)
    except Exception as e: print(f"  add_tone: {type(e).__name__}: {e}")
    # Sendungsfolgen markieren, damit das Frontend sie aus den Top-Meldungen
    # heraushalten kann (im Feed bleiben sie auffindbar).
    for a in deduped:
...
# Die neuen Gerichtsfeeds (BVerwG, BAG, BSG, BFH, EuG) laufen im selben Topf
# wie Bundestag und Bundesrat. Die Zuweisung steht hier, weil GERICHTS_FEEDS
# weiter oben im Anreicherungsteil definiert wird; gelesen wird die Liste
# ohnehin erst in main().
BUNDESTAG_FEEDS += GERICHTS_FEEDS
